...
public class Bytes implements Comparable<Bytes> {
  ...
  /**
   * Lexicographically compare a left array argument with a right array argument as a prefix,
   * limiting the number of bytes compared from the left argument to the length of the right
   * (prefix) argument. Typically used to place a key in a range where one wants to get a
   * lexicographical prefix match rather than a whole word match.
   *
   * @param left left operand
   * @param prefix right operand (prefix to match)
   * @return 0 if equal, < 0 if left is less than right, etc.
   */
  public static int compareToPrefix(final byte[] left, final byte[] prefix) {
    return LexicographicalComparerHolder.BEST_COMPARER.compareTo(left, 0,
        (prefix.length < left.length ? prefix.length : left.length), prefix, 0, prefix.length);
  }

  /**
   * Convert a long value to a byte array using little-endian.
   *
   * @param val value to convert
   * @return the byte array
   */
  public static byte[] longToBytesFlipped(long val) {
    byte[] b = new byte[8];
    for (int i = 0; i < 7; i++) {
      b[i] = (byte) val;
      val >>>= 8;
  ...
  /**
   * Convert an int value to a byte array in little endian.
   *
   * @param val value
   * @return the byte array
   */
  public static byte[] intToBytesFlipped(int val) {
    byte[] b = new byte[4];
    for (int i = 0; i < 4; i++) {
      b[i] = (byte) val;
      val >>>= 8;
